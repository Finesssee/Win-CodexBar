...
impl MiniMaxRegion {
    ...
    fn token_plan_usage_url(self) -> String {
        format!(
    ...
    fn token_plan_usage_summary_url(self) -> String {
        format!(
    ...
    fn token_plan_credit_url(self) -> String {
        format!("{}/backend/account/token_plan_credit", self.www_base_url())
    }
...
impl MiniMaxProvider {
    ...
    /// coding-plan error.
    async fn fetch_token_plan_with_cookie(
        &self,
        cookie_header: &str,
        region: MiniMaxRegion,
    ) -> Result<ProviderFetchResult, ProviderError> {
        ...
        let usage_snapshot = self
            .fetch_token_plan_usage_once(cookie_header, region, now)
            .await?;

        // (b)/(c) console backend views — best-effort, every failure → absent.
        let summary = self
            .fetch_token_plan_backend_json(
                cookie_header,
                &region.token_plan_usage_summary_url(),
                region,
            )
            .await
            .and_then(|json| parse_token_plan_summary(&json));
        let title = self
            .fetch_token_plan_backend_json(cookie_header, &region.token_plan_credit_url(), region)
            .await
    ...
    async fn fetch_token_plan_usage_once(
        ...
        let url = region.token_plan_usage_url();
        let response = self.token_plan_get(cookie_header, &url, region).await?;
    ...
    async fn fetch_token_plan_backend_json(
        ...
        let response = match self.token_plan_get(cookie_header, url, region).await {
    ...
    async fn fetch_with_cookie(
            ...
            .await
        {
            ...
            Err(err) if coding_plan::is_token_plan_without_coding_plan(&err) => {
                ...
                match self
                    .fetch_token_plan_with_cookie(cookie_header, region)
                    .await
                {
...
mod tests {
    ...
    /// Issue #254 — Token Plan (console) fallback: summary/title parsing and
    /// result assembly for China-mainland Token Plan accounts whose legacy
    /// coding-plan endpoint answers base_resp 2062.
    mod token_plan_tests {
        use super::*;
        ...
        fn assembles_full_token_plan_result() {
            let snapshot = coding_plan::parse_coding_plan_value(
                &reporter_token_plan_usage_json(),
                fixed_now(),
            )
            .unwrap();
            let summary = parse_token_plan_summary(&reporter_usage_summary_json()).unwrap();
        ...
        fn all_absent_falls_back_to_original_coding_plan_error() {
            ...
            let err =
                assemble_token_plan_result(None, None, None, original, fixed_now()).unwrap_err();
            match err {
